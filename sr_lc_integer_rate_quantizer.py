...
#        values that match the source distribution.
        ...
        #self.y = self.c_sf * stats.uniform.ppf(q, loc = mu, scale = s)
        ##SS2SR: will change the above two lines to have different distortion metrics.
        q = [(2*label + 1)/(2*(2**memory)) for label in self.labels]
        y_init = self.c_sf * stats.uniform.ppf(q, loc = mu, scale = s)
        t_init = np.zeros(len(y_init)-1)
        t_init[0] = y_init[0]-0.5
        for i in range(1,len(t_init)):
            t_init[i] = 0.5*(y_init[i-1]+y_init[i])
        self.y, self.threshold, self.errormaxl = maxlloyd(t_init, y_init, 0.0001)
        self.num_to_state = {}
